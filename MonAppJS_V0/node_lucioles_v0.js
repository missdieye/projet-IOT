...
async function main() {
  ...
  const mongoUri =
    "mongodb+srv://Binta:Dioudere@cluster0.r6mmx.mongodb.net/lucioles?retryWrites=true&w=majority"; // connection  URI
  ...
  mg_client.connect(function (err, mg_client) {
    ...
    console.log(
      'Mongo : Collection "temp" and "light" created or erased (dropped) !'
    );

    //===============================================
    // Connexion au broker MQTT distant
    //
    const mqtt_url = "test.mosquitto.org";
    ...
    client_mqtt.on("connect", function () {
      ...
      client_mqtt.subscribe(TOPIC_LIGHT, function (err) {
        if (!err) {
          ...
          console.log(
            'MQTT : Node Server has subscribed to "',
            TOPIC_LIGHT,
            '"'
          );
        }
    ...
    client_mqtt.on("message", function (topic, message) {
      ...
      dbo.collection(key).insertOne(new_entry, function (err, res) {
        ...
        console.log(
          "\nMONGO : \t Item : ",
          new_entry,
          "\nhas been inserted in db in collection :",
          key
        );
      });
