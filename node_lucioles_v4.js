...
async function v0() {
  ...
  const mongoUri =
    "mongodb+srv://missjey:Shuinydija1@cluster0.okvli.mongodb.net/lucioles?retryWrites=true&w=majority"; //URL de connection
  ...
  mg_client.connect(function (err, mg_client) {
    ...
    var client_mqtt = mqtt.connect("mqtt://test.mosquitto.org", {
    ...
    client_mqtt.on("message", function (topic, message) {
      ...
      if (index === -1) {
        wholist.push({
          who: wh,
          lastConnect: moment().format("MMMM DD YYYY, h:mm:ss"),
        });
      }
...
app.post("/signup", function (req, res) {
  ...
  if (goodMac) {
    dbo
      .collection("users")
      .findOne({ macEsp: req.body.macEsp }, function (err, result) {
        if (err) throw err;
        if (result) {
...
app.post("/login", function (req, res) {
  ...
  if (search_user.username == "admin" && search_user.macEsp == "admin") {
    ...
    dbo
      .collection("users")
      .findOne(
        { username: search_user.username, macEsp: search_user.macEsp },
        function (err, result) {
          if (err) throw err;
          if (result) {
            ...
            alert(
              "Votre nom d'utilisateur ou votre MAC address n'est pas reconnu."
            );
            res.redirect("/login.html");
          }
        }
...
app.get("/permissionAdmin", function (req, res) {
  dbo
    .collection("users")
    .findOne({ macEsp: req.session.macEsp }, function (err, result) {
      if (err) throw err;
      res.send(result);
    });
});
...
app.post("/accepter/:macEsp", function (req, res) {
  dbo
    .collection("users")
    .updateOne(
      { macEsp: req.params.macEsp },
      { $set: { permission_admin: true } },
      function (err, result) {
        if (err) throw err;
        res.redirect("/registeredUsers.html");
      }
...
app.post("/supprimer", function (req, res) {
  dbo
    .collection("users")
    .deleteOne({ macEsp: req.session.macEsp }, function (err, result) {
      if (err) throw err;
    });
